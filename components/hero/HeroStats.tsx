...
type Currency = NonNullable<NonNullable<NonNullable<ReturnType<typeof useCollections>['data']>[0]['topBid']>['price']>['currency']
...
const HeroStats: FC<{ stats: Props }> = ({ stats }) => {
  return (
    <div className='grid min-w-full grid-cols-2 items-center gap-[1px] overflow-hidden rounded-lg border-[1px] border-gray-300 bg-gray-300 dark:border-[#525252] dark:bg-[#525252] md:m-0 md:h-[82px] md:min-w-[647px] md:grid-cols-4 md:gap-2 md:bg-white dark:md:bg-black'>
      <Stat name='items'>
        <h3 className='reservoir-h6 dark:text-white'>{formatNumber(stats.count)}</h3>
      </Stat>
      <Stat name='top offer'>
        <h3 className='reservoir-h6 dark:text-white'>
          <FormatCrypto amount={stats.topOffer} decimals={stats.topOfferCurrency?.decimals} address={stats.topOfferCurrency?.contract} />
        </h3>
      </Stat>
      <Stat name='floor'>
...
export const PercentageChange: FC<{ value: number | undefined | null }> = ({ value }) => {
  ...
  if (value < 1) {
    return <div className='text-sm text-[#FF3B3B]'>{formatNumber(percentage)}%</div>
  }

  if (value > 1) {
    return <div className='text-sm text-[#06C270]'>+{formatNumber(percentage)}%</div>
  }
